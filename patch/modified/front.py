    ...
    def load(self):
        ...
        if platform.system() != "Linux":
            ...
            try:
                ...
                self.zh_normalizer = _DummyNormalizer()
                self.en_normalizer = _DummyNormalizer()
...
class TextTokenizer:
    ...
    def split_sentences_by_token(
        ...
        merge_sentences=False：不合并相邻短句，每个标点句独立成段
        （句号=段界，供段间停顿控制；默认 True 保持官方行为）
        """
        # 处理特殊情况
    ...
    def split_sentences(self, tokenized: List[str], max_tokens_per_sentence=120,
        ...
        return TextTokenizer.split_sentences_by_token(
            ...
            max_tokens_per_sentence=max_tokens_per_sentence, merge_sentences=merge_sentences,
        )
