...
class IndexTTS2:
    def __init__(
            ...
            use_cuda_kernel=None,use_deepspeed=False, low_vram=False, use_accel=False, use_torch_compile=False
    ):
        """
        Args:
        ...
        if self.low_vram:
            ...
            # If low_vram is on, prefer fp16 if available and not explicitly disabled (though here we just force it if generic logic allows)
            # Actually, let's just let the below logic handle device selection, but if it's cuda, we want fp16.
            # However, use_fp16 arg is passed in. We should probably override it if low_vram is True and we are on cuda.
            pass
        ...
        if self.low_vram and self.device.startswith("cuda"):
            if not self.use_fp16:
                print(">> Low VRAM mode: Forcing FP16 on CUDA.")
                self.use_fp16 = True

        self.max_ref_audio_duration = 8 if self.low_vram else 15
        ...
        self.qwen_emo = QwenEmotion(os.path.join(self.model_dir, 'hub', self.cfg.qwen_emo_path))
        ...
        if self.use_fp16:
            ...
            self.gpt.eval()
        # Low VRAM: Move back to CPU after init
        ...
        facebook_local_root = os.path.join(self.model_dir, "hub", "models--facebook--w2v-bert-2.0")
        facebook_snapshots_dir = os.path.join(facebook_local_root, "snapshots")
        local_feature_path = None
        if os.path.isdir(facebook_snapshots_dir):
            try:
                subdirs = [d for d in os.listdir(facebook_snapshots_dir)
                          if os.path.isdir(os.path.join(facebook_snapshots_dir, d))]
                if subdirs:
                    local_feature_path = os.path.join(facebook_snapshots_dir, subdirs[0])
            except Exception:
                local_feature_path = None

        if local_feature_path is not None and os.path.isdir(local_feature_path):
        ...
        # Low VRAM: Move back to CPU after init
        self.offload_model(self.semantic_model)

        semantic_codec = build_semantic_codec(self.cfg.semantic_codec)
        amphion_root = os.path.join(self.model_dir, "hub", "models--amphion--MaskGCT", "snapshots")
        semantic_code_ckpt = None
        if os.path.isdir(amphion_root):
            try:
                subdirs = [d for d in os.listdir(amphion_root)
                          if os.path.isdir(os.path.join(amphion_root, d))]
                if subdirs:
        ...
        if semantic_code_ckpt is None or not os.path.isfile(semantic_code_ckpt):
            semantic_code_ckpt = os.path.join(
                self.model_dir,
                "amphion_MaskGCT",
                "semantic_codec",
        ...
        # Low VRAM: Move back to CPU after init
        self.offload_model(self.semantic_codec)
        ...
        # Low VRAM: Move back to CPU after init
        self.offload_model(self.campplus_model)
        print(">> campplus_model initialized without pretrained weights")

        bigvgan_name = self.cfg.vocoder.name
        # 优先使用本地 BigVGAN 模型
        local_bigvgan_path = os.path.join(self.model_dir, "hub", "models--nvidia--bigvgan_v2_22khz_80band_256x")

        local_snapshot_dir = None
        if hasattr(self.cfg, "bigvgan_checkpoint"):
            ckpt_full = os.path.join(self.model_dir, self.cfg.bigvgan_checkpoint)
            snap_dir = os.path.dirname(ckpt_full)
            if os.path.isdir(snap_dir):
                local_snapshot_dir = snap_dir

        if os.path.isdir(local_bigvgan_path):
            self.bigvgan = bigvgan.BigVGAN.from_pretrained(
                local_bigvgan_path,
                use_cuda_kernel=self.use_cuda_kernel,
            )
            print(f">> bigvgan loaded from local path: {local_bigvgan_path}")
        elif local_snapshot_dir is not None:
            self.bigvgan = bigvgan.BigVGAN.from_pretrained(
                local_snapshot_dir,
                use_cuda_kernel=self.use_cuda_kernel,
            )
            print(f">> bigvgan loaded from local snapshot: {local_snapshot_dir}")
        else:
            # 最后兜底：走 HuggingFace 远程（需要联网）
            self.bigvgan = bigvgan.BigVGAN.from_pretrained(
                bigvgan_name,
                use_cuda_kernel=self.use_cuda_kernel,
            )
            print(f">> bigvgan loaded from HuggingFace: {bigvgan_name}")

        self.bigvgan = self.bigvgan.to(self.device)
        self.bigvgan.remove_weight_norm()
        self.bigvgan.eval()
        # Low VRAM: Move back to CPU after init
        self.offload_model(self.bigvgan)
    ...
    def get_emb(self, input_features, attention_mask):
        try:
            dev = next(self.semantic_model.parameters()).device
    ...
    def offload_model(self, model):
        """Helper to move model to CPU and clear cache if low_vram is enabled"""
        if self.low_vram and model is not None:
            model.to("cpu")
            if self.device.startswith("cuda"):
                torch.cuda.empty_cache()

    def onload_model(self, model):
        """Helper to move model back to GPU if low_vram is enabled"""
        if self.low_vram and model is not None:
            if self.use_fp16 and self.device.startswith("cuda") and model is not self.semantic_codec:
                 # Note: semantic_codec usually stays in fp32 or as loaded, others might need half()
                 # But here we just move to device. The model should already be in correct dtype if possible.
                 # However, if it was moved to CPU, it might have been cast to float32 automatically or kept as half.
                 # Safest is just .to(self.device).
                 pass
    ...
    def infer(self, spk_audio_prompt, text, output_path,
              ...
              emo_vector=None,
              use_emo_text=False, emo_text=None, use_random=False, interval_silence=400,
              speaking_speed=1.0, verbose=False, max_text_tokens_per_segment=120, seed=None, dump_codes_path=None,
              pause_mode=False, detect_cfm_steps=25, **generation_kwargs):
        print(">> starting inference...")
        self._set_gr_progress(0, "starting inference...")

        # 设置或生成 seed
        if seed is None:
            # 未指定 seed：抽取随机种子并显式设置，保证记录值可复现本次生成。
            # 不能用 torch.initial_seed()：manual_seed 之后它恒返回上次设置的值。
        ...
        if self.cache_spk_cond is None or self.cache_spk_audio_prompt != spk_audio_prompt:
            if self.cache_spk_cond is not None:
                ...
                self.cache_mel = None
                torch.cuda.empty_cache()
            audio,sr = self._load_and_cut_audio(spk_audio_prompt,self.max_ref_audio_duration,verbose)
            ...
            # Low VRAM: Ensure semantic model is on GPU
            self.onload_model(self.semantic_model)
            spk_cond_emb = self.get_emb(input_features, attention_mask)
            self.offload_model(self.semantic_model)

            # Low VRAM: Ensure semantic codec is on GPU
            self.onload_model(self.semantic_codec)
            ...
            # Low VRAM: Ensure campplus model is on GPU
            self.onload_model(self.campplus_model)
            style = self.campplus_model(feat.unsqueeze(0))  # 参考音频的全局style2[1,192]
            self.offload_model(self.campplus_model)

            # Low VRAM: Ensure s2mel is on GPU for length_regulator
            self.onload_model(self.s2mel)
        ...
        if self.cache_emo_cond is None or self.cache_emo_audio_prompt != emo_audio_prompt:
            if self.cache_emo_cond is not None:
                self.cache_emo_cond = None
                torch.cuda.empty_cache()
            emo_audio, _ = self._load_and_cut_audio(emo_audio_prompt,self.max_ref_audio_duration,verbose,sr=16000)
        ...
        if pause_mode:
            ...
            print(f">> [pause] 检测到 {len(pause_marks)} 个停顿标记，已转逗号（全波形精确控制）")
        else:
            pause_marks = []
        ...
        for seg_idx, sent in enumerate(segments):
            ...
            with torch.no_grad():
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    # Low VRAM: Ensure gpt is on GPU
                    self.onload_model(self.gpt)
                ...
                if dump_codes_path:
                    torch.save(codes.detach().cpu(), dump_codes_path)
                    print(f">> codes dumped to: {dump_codes_path}")
                if verbose:
                    print(codes, type(codes))
                ...
                seg_marks = seg_marks_all[seg_idx] if seg_marks_all is not None else []
                ...
                if seg_marks:
                    ...
                    with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                        ...
                        gpt_forward_time += time.perf_counter() - m_start_time
                        # Low VRAM: Done with gpt, offload it
                        self.offload_model(self.gpt)
                    wav = self._s2mel_wav(latent, codes, code_lens, diffusion_steps, inference_cfg_rate,
                                          speaking_speed, prompt_condition, ref_mel, style)
        ...
        if output_path:
            ...
            # torchaudio.save 在 2.9 中强制走 torchcodec（需要系统 FFmpeg DLL），改用 soundfile
    ...
    def _s2mel_wav(self, latent, codes, code_lens, diffusion_steps, inference_cfg_rate,
                   speaking_speed, prompt_condition, ref_mel, style):
        """codes+latent → wav（gpt_layer → vq2emb → length_regulator → cfm → bigvgan）。
        常规路径与全波形（检测/波形操作）复用。返回 (1, samples) wav，未 clamp。
        """
        dtype = None
        with torch.amp.autocast(latent.device.type, enabled=dtype is not None, dtype=dtype):
            self.onload_model(self.s2mel)
            ...
            speed = speaking_speed if speaking_speed else 1.0
            target_lengths = (code_lens * 1.72 / max(0.001, speed)).long()
            cond = self.s2mel.models['length_regulator'](S_infer, ylens=target_lengths,
            ...
            self.offload_model(self.s2mel)
            self.onload_model(self.bigvgan)
