...
1. **Install via ComfyUI-Manager**: the node is published on the official
   ...
   ⚠️ **Newly published nodes may not have entered Manager's search index yet
   (they can occasionally be missing from search)**; if you can't find it, use
   the manual method below: put this repository directory into ComfyUI's
   `custom_nodes/`
