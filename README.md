...
分句稿按**一句一段**切分（句号在段尾），句与句之间的停顿默认在拼接阶段统一
控制（段间插静音）；需要精确控制句号停顿的地方，用标记直接覆盖：
`句子[pause:800ms]。`（句号前）或 `句子。[pause:800ms]`（句号后）均精确。
...
- **句号前标记处模型常只产生弱停顿**（无 ≥30ms 真静音核心），"最近核心"策略会
  命中相邻的真实停顿（句号边界）——实测 800ms 目标 → 838ms（位置略偏、听感正确）；
  若窗口内完全无静音则操作被拒绝（宁缺毋滥）
