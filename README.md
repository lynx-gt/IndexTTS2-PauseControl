...
本项目的核心目的：**为 IndexTTS2 提供毫秒级精确停顿控制（`[pause:N]`）**——
在文本任意标点处指定停顿时长，生成音频中对应停顿被精确调整为目标值
（实测 20 个片段 40 标记平均偏差 13ms，最大 32ms）。

> **术语**：**分句**（segment）= 推理时按标点切出的标点句（官方 `split_sentences`
> 输出，代码变量 `segments`）；**片段** = 分句稿中的 `# 片段 N` 条目（批量生成
> 单位，一条 wav；一个片段可能含多个分句，片段内多句之间 = 分句间静音）。

- **`[pause:N]` 精确停顿**：句中与句号前后统一支持：
...
在实现精确停顿的基础上，为实际批量生产流程添加的能力（官方均无）：

- **批量生成 + 候选试听 + 验收标记**：manifest 断点续跑、rounds 轮次候选、试听节点一键标记"第 N 轮通过验收"
- **seed 固定/复现**：固定 seed 可精确复现同一生成；未固定时自动记录实际 seed 到 `.seed` 文件
...
`IndexTTSBatch` 运行后产生一个**任务目录**（`output/{tag}_{时间戳}/`，
或自定义 `output_dir`）：
...
└── manifest.json    任务参数 + 每片段每轮的 seed/时长/验收标记（断点续跑依据）
```

- 文件命名：`{片段序号:03d}_{轮次}.wav`（`001_1` = 片段 1 第 1 轮）
- 用 `IndexTTSListen` 节点试听候选，`accept_round` 标记"第 N 轮通过验收"
  （写入 manifest.json，供后续拼接流程选用）
