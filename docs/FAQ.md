...
**Q：句号处怎么控制停顿？**
分句按句号切段（句号=段界），句号停顿默认 = 段间静音（`interval_silence`，
默认 400ms，稳定规整）；需要精确覆盖时用标记：
`句子[pause:800ms]。`（句号前）或 `句子。[pause:800ms]`（句号后），两者均精确
（段尾兜底：段内未命中自动转段间控制；最后一段在音频末尾追加静音）。
引号场景（`…。'`）同样支持。
...
**Q：实测精度多少？**
验收 20 段 40 标记平均偏差 13ms，最大 32ms；回归 5 段平均 5ms；
6 段压力测试（段内/句号前/句号后/引号/无标记）每段 3 候选全部命中。
